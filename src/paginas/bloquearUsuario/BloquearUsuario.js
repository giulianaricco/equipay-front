...
const BloquearUsuario = () => {
  ...
  const obtenerUsuarios = async () => {
    try {
      ...
      if (response.status === 200) { 
        const usuariosFiltrados = response.data.filter(usuario => usuario.estadoUsuario !== 'ELIMINADO' && usuario.estadoUsuario !== 'BLOQUEADO');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
        const response = await axios.post(`/api/usuarios/${idUsuario}/bloquear`, null, {
        headers: {
            'Authorization': `Bearer ${token}`
        },
        });
      ...
      if (response.status === 200) {
        ...
        toastr.success("Usuario bloqueado exitosamente.");
